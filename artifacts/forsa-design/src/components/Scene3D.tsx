...
/**
 * WebGL preflight. When the browser cannot create a GL context (no GPU,
 * hardware acceleration disabled, headless environments), the 3D scene is
 * skipped entirely instead of throwing — otherwise the global error boundary
 * replaces the whole About page with the "Something went wrong" fallback.
 */
...
export default function Scene3D() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Match the page background (--background) so the canvas blends with the site.
      const bgValue = getComputedStyle(document.documentElement)
        .getPropertyValue("--background")
        .trim();
      scene.background = bgValue ? new THREE.Color(`hsl(${bgValue.split(" ").join(", ")})`) : null;
    ...
    `;
      ...
      // Renderer init failed (WebGL unavailable, driver exhausted, etc.).
      // Degrade gracefully: clear any partial canvas and keep the page alive
      // instead of letting the error boundary blank the whole route.
